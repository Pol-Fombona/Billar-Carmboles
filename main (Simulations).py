...
class GraphicsEngine(Engine):
    ...
    def simulate_IA_turn(self, difficulty):
        # Simulates turn made by IA

        pg.display.set_caption("Processing IA turn...")
        turn_data = make_turn(self.scene.ball_objects, self.game, difficulty)
    ...
    def run(self):
        ...
        last_timestamp = time.time()
        match_name = 1
        self.game.game_speed = 240
        simulation_data = []

        print("Options:", "Max Score:",self.game.mode.max_score, "- Max Turns:", self.game.mode.max_turn)

        while True:
            print("Match", match_name, "in progress")
            match_ended = False
            last_timestamp = time.time()
            self.game.played_time = 0
            self.game.current_player = self.game.player1
            difficulty_p1 = random.choice(["Easy", "Normal", "Hard"])
            difficulty_p2 = random.choice(["Easy", "Normal", "Hard"])
            print("Difficuly P1:", difficulty_p1, "- P2:", difficulty_p2)
            self.game.player1.score = 0
            self.game.player1.played = False
            self.game.player1.collision_record = []
            self.game.player1.turn_count = 1

            self.game.player2.score = 0
            self.game.player2.played = False
            self.game.player2.collision_record = []
            self.game.player2.turn_count = 1

            first_sphere_position = (30, 1, 10)
            second_sphere_position = (10, 1, 10)
            third_sphere_position = (20, 1, 60)
            self.game.spheres[0].pos = first_sphere_position
            self.game.spheres[1].pos = second_sphere_position
            self.game.spheres[2].pos = third_sphere_position
                ...
                if self.pause: #and self.game.current_player.type != "IA":
                    ...
                    if turn_status == "IA":
                        if self.game.current_player == self.game.player1:
                            difficulty = difficulty_p1
                        else:
                            difficulty = difficulty_p2

                        self.simulate_IA_turn(difficulty)
                        self.save_game_data(turn_status, match_name)
                        ...
                        if turn_status == "played":
                            ...
                            self.render()
                            scored = self.game.mode.update_score(self.game.current_player)
                            self.save_game_data(turn_status, score = scored, match = match_name)
                            self.game.changeCurrentPlayer(scored)
            ...
            if self.game.player1.score == self.game.player2.score:
                ...
                winner_dif = difficulty_p1
                win_reason = "Max Score" if self.game.player1.score == self.game.mode.max_score else "Max Turn"
                print("Result: Winner", self.game.player1.name)

            elif self.game.player1.score < self.game.player2.score:
                print("Result: Winner", self.game.player2.name)
                winner = self.game.player2.name
                winner_dif = difficulty_p2
                win_reason = "Max Score" if self.game.player2.score == self.game.mode.max_score else "Max Turn"

            
            data = [match_name, winner, winner_dif, win_reason, difficulty_p1, difficulty_p2, self.game.player1.score, self.game.player2.score, 
                    self.game.player1.turn_count, self.game.player2.turn_count]
            
            simulation_data.append(data)
            
            print("Scores ", self.game.get_scores(), "\n")

            if (match_name % 5) == 0:
                cols = ["Match", "Winner", "WinnerDifficulty", "WinReason", "P1_Difficulty", "P2_Difficulty", "P1_Score", "P2_Score", "P1_Turns", "P2_Turns"]
                df = pd.DataFrame(simulation_data, columns=cols)
                df.to_pickle("GameData/Simulations/match_results_data.pkl")

                columns = ["Match", "PlayerName", "TurnStatus", "TurnCount", "Pos1", "Pos2", "Pos3"]
                self.game_data = self.game_data[1:]
                df = pd.DataFrame(self.game_data, columns=columns)
                df.to_pickle("GameData/Simulations/match_complete_data.pkl")

            if match_name == 100:
                cols = ["Match", "Winner", "P1_Difficulty", "P2_Difficulty", "P1_Score", "P2_Score", "P1_Turns", "P2_Turns"]
                df = pd.DataFrame(simulation_data, columns=cols)
                df.to_pickle("GameData/Simulations/match_results_data.pkl")
    ...
    def save_game_data(self, turn_status, match = None, score = None):

        data = [match, self.game.current_player.name, turn_status, self.game.current_player.turn_count,
